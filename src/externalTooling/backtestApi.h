...
public:
    ...
    tradeData advanceIdx(float currentPrice) {
        if (!_lockTrade) {
            float diff = currentPrice - _entryPrice;
...
public:
    ...
    /// @param tickRes 1 = full resolution, how many ticks is the program allowed to skip to match with the timeframe
    std::vector<float> requestDataWindow(MarketData& md, int period, int timeframe=0, int tickRes=1) {
        std::vector<float> window;
        window.reserve(period);
        auto pushPrice = [&](std::string_view sv) {
            float px = 0.f;
...
std::vector<std::vector<float>> returnVolumeProfile(int anchor, std::vector<float>& prices,
    ...
    std::vector<std::vector<float>> passedPrices; // [[Price, Volume], [Price, Volume]...]
    for (int i=anchor; i<prices.size(); i++) {
        for (auto& pp : passedPrices) {
            // check if we already passed current price
            // if not, append it to passedPrices with the volume
            // if yes, don't append it but add volume to pp[1]
            if (prices[i] != pp[0]) { // if not 
                passedPrices.push_back(std::vector<float>{prices[i], volumeData[i]});
            } else { // if yes
                pp[1] += volumeData[i];
            } 
        }
    } return passedPrices; // passedPrices is essentially the profile,
                           // it isn't named as such because I figured it would
                           // reduce readability
} 
